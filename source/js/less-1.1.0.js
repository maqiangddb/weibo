                ...
                return function (options, variables) {
                    ...
                    try {
                        var css = evaluate.call(this, { frames: frames })
                                          .toCSS([], { compress: options.compress || false , getLine: getLine});
                    } catch (e) {
                        lines = input.split('\n');
        ...
        for (var i = 0; i < this.rules.length; i++) {
            ...
            if (rule.rules || (rule instanceof tree.Directive)) {
                ...
                if (rule.toCSS && !rule.variable) {
                    var lineNo = env.getLine(rule.index) + 1; // why 1 or 2?
                    rules.push(rule.toCSS(env) + '/* ' + lineNo + ' */'); // add line no?
...
cache = null; // so we will generate less everytime, add by xc
